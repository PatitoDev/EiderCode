...
public class FileIconProvider
{
    ...
    {
        HasLoaded = false;
        var themeJson = GD.Load<Json>($"res://EiderCode.UI/Icons/catppuccin/{_iconThemeKeyMap[iconThemeKey]}/theme.json");
        ...
        _iconTextureMap = _iconTheme.IconDefinitions
          ...
          item =>
          {
              var icon = GD.Load<CompressedTexture2D>(
                "res://" + Path.Combine($"EiderCode.UI/Icons/catppuccin/{_iconThemeKeyMap[iconThemeKey]}", item.Value.IconPath)
              );
